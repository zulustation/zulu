...
use zulu_runtime::parameters::SS58Prefix;

use zulu_primitives::constants::zul::{LIQUIDITY_MINING, LIQUIDITY_MINING_PTD};
...
cfg_if::cfg_if! {
    if #[cfg(feature = "parachain")] {
        const DEFAULT_STAKING_AMOUNT_ZULU: u128 = MinCollatorStk::get();
        const DEFAULT_COLLATOR_BALANCE_ZULU: Option<u128> =
            DEFAULT_STAKING_AMOUNT_ZULU.checked_add(CollatorDeposit::get());
        pub type ZuluChainSpec = sc_service::GenericChainSpec<zulu_runtime::GenesisConfig, Extensions>;
    } else {
        pub type ZuluChainSpec = sc_service::GenericChainSpec<zulu_runtime::GenesisConfig>;
    }
}

fn endowed_accounts_staging_zulu() -> Vec<EndowedAccountWithBalance> {
    vec![
        ...
        EndowedAccountWithBalance(
            ...
            DEFAULT_COLLATOR_BALANCE_ZULU.unwrap(),
        ),
...
#[cfg(feature = "parachain")]
fn additional_chain_spec_staging_zulu(
...
fn additional_chain_spec_staging_zulu() -> AdditionalChainSpec {
    AdditionalChainSpec {
...
pub(super) fn get_wasm() -> Result<&'static [u8], String> {
    zulu_runtime::WASM_BINARY.ok_or_else(|| "WASM binary is not available".to_string())
}

generate_generic_genesis_function!(zulu_runtime,);

#[cfg(feature = "parachain")]
generate_inflation_config_function!(zulu_runtime);

pub fn zulu_staging_config() -> Result<ZuluChainSpec, String> {
    let wasm = get_wasm()?;

    Ok(ZuluChainSpec::from_genesis(
        "Zulu Staging",
        "zulu_staging",
        ChainType::Live,
        move || {
            generic_genesis(
                additional_chain_spec_staging_zulu(
                    #[cfg(feature = "parachain")]
                    KUSAMA_PARACHAIN_ID.into(),
                ),
                endowed_accounts_staging_zulu(),
                wasm,
            )
        },
        vec![],
        ...
        Some("zulu"),
        None,
        Some(token_properties("ZUL", SS58Prefix::get())),
        #[cfg(feature = "parachain")]
