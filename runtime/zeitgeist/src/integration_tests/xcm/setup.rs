...
use crate::{
    xcm_config::config::{general_key, zulu},
...
impl Default for ExtBuilder {
    fn default() -> Self {
        Self { balances: vec![], parachain_id: zulu::ID }
    }
}
...
pub(super) fn foreign_zul_multilocation() -> MultiLocation {
    MultiLocation::new(1, X2(Parachain(zulu::ID), general_key(zulu::KEY)))
}

#[inline]
pub(super) fn foreign_sibling_multilocation() -> MultiLocation {
    MultiLocation::new(1, X2(Parachain(PARA_ID_SIBLING), general_key(zulu::KEY)))
}
