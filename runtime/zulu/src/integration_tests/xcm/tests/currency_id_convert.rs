...
use crate::{
    integration_tests::xcm::{
        setup::{
            foreign_parent_multilocation, foreign_sibling_multilocation, foreign_zul_multilocation,
            register_foreign_parent, register_foreign_sibling, FOREIGN_PARENT_ID,
            FOREIGN_SIBLING_ID,
        },
        test_net::Zulu,
    },
    xcm_config::config::{general_key, zulu, AssetConvert},
    CurrencyId,
};
...
#[test]
fn convert_native() {
    assert_eq!(zulu::KEY.to_vec(), vec![0, 1]);

    // The way Zul is represented relative within the Zulu runtime
    let zul_location_inner: MultiLocation = MultiLocation::new(0, X1(general_key(zulu::KEY)));

    assert_eq!(<AssetConvert as C1<_, _>>::convert(zul_location_inner), Ok(CurrencyId::Zul));

    // The canonical way Zul is represented out in the wild
    Zulu::execute_with(|| {
        assert_eq!(
            <AssetConvert as C2<_, _>>::convert(CurrencyId::Zul),
            Some(foreign_zul_multilocation())
        )
    });
}
...
fn convert_any_registered_parent_multilocation() {
    Zulu::execute_with(|| {
        ...
        // Register parent as foreign asset in the Zulu parachain
...
fn convert_any_registered_sibling_multilocation() {
    Zulu::execute_with(|| {
        ...
        // Register sibling as foreign asset in the Zulu parachain
...
fn convert_unkown_multilocation() {
    let unknown_location: MultiLocation =
        MultiLocation::new(1, X2(Parachain(zulu::ID), general_key(&[42])));
