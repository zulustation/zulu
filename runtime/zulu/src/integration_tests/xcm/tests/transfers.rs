...
use crate::{
    integration_tests::xcm::{
        setup::{
            ksm, register_foreign_parent, register_foreign_zul, sibling_parachain_account,
            zulu_parachain_account, zul, ALICE, BOB, FOREIGN_PARENT_ID, FOREIGN_ZUL_ID,
            PARA_ID_SIBLING,
        },
        test_net::{KusamaNet, Sibling, TestNet, Zulu},
    },
    xcm_config::{config::zulu, fees::default_per_second},
...
#[test]
fn transfer_zul_to_sibling() {
    TestNet::reset();

    let alice_initial_balance = zul(10);
    let transfer_amount = zul(5);
    let mut treasury_initial_balance = 0;

    Sibling::execute_with(|| {
        treasury_initial_balance =
            Tokens::free_balance(FOREIGN_ZUL_ID, &ZuluTreasuryAccount::get());
        assert_eq!(Tokens::free_balance(FOREIGN_ZUL_ID, &BOB.into()), 0);
        register_foreign_zul(None);
    });
    ...
    Sibling::execute_with(|| {
        let current_balance = Tokens::free_balance(FOREIGN_ZUL_ID, &BOB.into());

        // Verify that BOB now has (amount transferred - fee)
        assert_eq!(current_balance, transfer_amount - zul_fee());
        ...
        assert_eq!(
            Tokens::free_balance(FOREIGN_ZUL_ID, &ZuluTreasuryAccount::get()),
            treasury_initial_balance + zul_fee()
        )
    });
}

#[test]
fn transfer_zul_sibling_to_zulu() {
    TestNet::reset();

    // In order to be able to transfer ZUL from Sibling to Zulu, we need to first send
    // ZUL from Zulu to Sibling, or else it fails since it'd be like Sibling had minted
    // ZUL on their side.
    transfer_zul_to_sibling();

    let alice_initial_balance = zul(5);
    let bob_initial_balance = zul(5) - zul_fee();
    let mut treasury_initial_balance = 0;
    let sibling_sovereign_initial_balance = zul(5);
    let transfer_amount = zul(1);
    // Note: This asset was registered in `transfer_zul_to_sibling`

    Zulu::execute_with(|| {
        treasury_initial_balance = Balances::free_balance(ZuluTreasuryAccount::get());
    ...
    Sibling::execute_with(|| {
        assert_eq!(Balances::free_balance(&zulu_parachain_account()), 0);
        assert_eq!(Tokens::free_balance(FOREIGN_ZUL_ID, &BOB.into()), bob_initial_balance);
        assert_ok!(XTokens::transfer(
        ...
        assert_eq!(
            Tokens::free_balance(FOREIGN_ZUL_ID, &BOB.into()),
    ...
    Zulu::execute_with(|| {
        ...
        assert_eq!(
            ...
            alice_initial_balance + transfer_amount - zul_fee(),
        );
        ...
        assert_eq!(
            Balances::free_balance(ZuluTreasuryAccount::get()),
            treasury_initial_balance + zul_fee()
        )
    });
}
...
fn transfer_ksm_from_relay_chain() {
    ...
    KusamaNet::execute_with(|| {
        ...
        assert_ok!(kusama_runtime::XcmPallet::reserve_transfer_assets(
            ...
            Box::new(Parachain(zulu::ID).into().into()),
...
#[test]
fn transfer_zul_to_sibling_with_custom_fee() {
    TestNet::reset();

    let alice_initial_balance = zul(10);
    // 10x fee factor, so ZUL has 10x the worth of sibling currency.
