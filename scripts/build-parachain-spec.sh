...
cargo build --profile=production --bin zulu --features parachain
./target/production/zulu build-spec --chain $STAGE_CHAIN --disable-default-bootnode > $OUTPUT_FILE
...
./target/production/zulu build-spec --chain $OUTPUT_FILE --disable-default-bootnode --raw > $OUTPUT_FILE.raw
rm -f $OUTPUT_FILE
